...
export function TrendsChart({ data }: TrendsChartProps) {
  const chartData = data.data.map((point, index) => ({
    ...
    actual: point.total > 0 ? point.total : null,
    trend: index < data.trend_line.length ? data.trend_line[index] : undefined,
  }));

  const hasData = data.data.some(p => p.total > 0);
            ...
            <ResponsiveContainer>
              <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <Tooltip
                  ...
                  labelStyle={{ color: 'var(--color-text-secondary)', marginBottom: '0.2rem' }}
                />
                <Legend wrapperStyle={{ color: '#94a3b8' }} />
                <Line type="monotone" dataKey="actual" stroke="#22d3ee" strokeWidth={2} dot={{ r: 4, fill: '#22d3ee' }} name="Фактические" connectNulls={false} />
                <Line type="monotone" dataKey="trend" stroke="#818cf8" strokeWidth={2} strokeDasharray="5 5" dot={false} name="Тренд" connectNulls />
              </LineChart>
            </ResponsiveContainer>
          </div>
